...
class PosecellNetwork:
    ...
    # set up the wrap lookups
        ...
        self.posecells[self.best_x][self.best_y][self.best_th] = 1;
    ...
    def path_integration(self,vtrans,vrot):
        ...
        for dir_pc in range(0,self.PC_DIM_TH):
            ...
            self.rot90_square(self.posecells[dir_pc], self.PC_DIM_XY, (int)math.floor((dir*2.0)/math.pi))
            dir90=dir-math.floor(dir*2/math.pi)*math.pi/2
            pca_new_memory=bytearray((self.PC_DIM_XY+2)**2)
            for j in range(self.PC_DIM_XY):
                pca_new_rot_ptr[j+1][1]=self.posecells[dir_pc][j][0]
            ...
            pca_new_rot_ptr2[0][0]=pca_new_rot_ptr[0][0]*weight_ne+pca_new_rot_ptr[0][self.PC_DIM_XY+1]*weight_se+pca_new_rot_ptr[self.PC_DIM_XY+1][0]*weight_nw
            for i in range(1,self.PC_DIM_XY+2):
                pca_new_rot_ptr2[j][0]=pca_new_rot_ptr[j][0]*weight_ne+pca_new_rot_ptr[j][self.PC_DIM_XY+1]*weight_se+pca_new_rot_ptr[j-1][0]*weight_nw
                for i in range(self.PC_DIM_XY+2):
                    pca_new_rot_ptr2[j][i]=pca_new_rot_ptr[j][i]*weight_ne+pca_new_rot_ptr[j][i-1]*weight_se+pca_new_rot_ptr[j-1][i]*weight_nw
            for i in range((self.PC_DIM_XY+2)**2):
                pca_new_rot_ptr2[0][i]+=pca_new_rot_ptr[0][i]*weight_sw
                
            for j in range(self.PC_DIM_XY):
                for i in range(self.PC_DIM_XY):
                    self.posecells[dir_pc][j][i]=pca_new_rot_ptr2[j+1][i+1]
                    
                    
            for i in range(self.PC_DIM_XY):
                self.posecells[dir_pc][0][i]+=pca_new_rot_ptr2[self.PC_DIM_XY+1][i+1]
            for i in range(self.PC_DIM_XY):
                self.posecells[dir_pc][j][0]+=pca_new_rot_ptr2[j+1][self.PC_DIM_XY+1]
            
            
            self.posecells[dir_pc][0][0]+=pca_new_rot_ptr2[self.PC_DIM_XY+1][self.PC_DIM_XY+1]
            
            
            self.posecells[dir_pc,:,:]=rot90(self.posecells[dir_pc,:,:],4-int(math.floor(dir*2.0/math.pi)))
            
        if vrot !=0:
            weight=math.abs(vrot)/self.PC_C_SIZE_TH
            while(weight>1):
                weight-=1
            if (weight==0):
                weight=1.0
            ...
            for i in range(self.PC_DIM_XY):
                for j in range(self.PC_DIM_XY):
                    self.posecells_memory[k]=self.posecells[i,j,:]    
                    k+=1
            ...
            if vrot<0:
                ...
                sign_vrot=-1
                sign_vrot=1
            shifty1=int(sign_vrot*math.floor(math.abs(vrot))/self.PC_C_SIZE_TH)
            shifty2=int(sign_vrot*math.ceil(math.abs(vrot)/self.PC_C_SIZE_TH))
            while(shifty1>0):
                ...
                for i in range(self.PC_DIM_XY):
                    for k in range(self.PC_DIM_TH):
                        ...
                        if newk1<self.PC_DIM_TH and newk2<self.PC_DIM_TH and newk1>=0 and newk2>=0:
                            temp=self.pca_new_memory[i][j]
                            self.posecells[k][j][i]=temp[newk1]*(1.0-weight)+temp[newk2]*weight
